...
/* 엔티티 이름을 적지 않으면 클래스의 이름을 엔티티 이름으로 가져간다. */
@Entity(name = "entityMember")
@Table(name = "tbl_member")

/* 클래스 레벨에 설정 가능하며 모든 필드를 대상으로 적용하겠다는 의미 -> FIELD: default */
...
public class Member {
    ...
    /* 필드 레벨에 설정 가능하며 해당 필드를 대상으로 적용하겠다는 의미 */
    @Access(AccessType.FIELD)
    @Column(
    ...
    /* ORDINAL: 숫자로 다루게 된다. 인덱스로 시작, STRING을 많이 사용함 */
