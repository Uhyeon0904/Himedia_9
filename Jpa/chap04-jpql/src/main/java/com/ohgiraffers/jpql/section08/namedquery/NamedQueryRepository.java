...
public class NamedQueryRepository {
    ...
    * 쿼리DSL 라이브러리를 사용하면 좀 더 간결하다. */
    ...
    public List<Menu> selectByNamedQuery() {
        List<Menu> menuList =
                entityManager.createNamedQuery("Section08Menu.selectMenuList", Menu.class)
                .getResultList();
        return menuList;
    }

    public List<Menu> selectByNamedQueryWithXml(int meuCode) {
        List<Menu> menuList =
                entityManager.createNamedQuery("Section08Menu.selectMenuByCode", Menu.class)
                .setParameter("menuCode", meuCode)
                .getResultList();
        return menuList;
    }
}
