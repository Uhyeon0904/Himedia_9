...
/*  어노테이션으로 설정 할 경우 클래스 위에 @WebServlet을 해줘야한다.   */
/*  loadOnStartUp 속성으로 우선순위를 지정할 수 있으며(숫자가 낮을 수록 우선순위가 높다.)
* 서버가 start 될 때 인스턴스를 생성하고 init()을 호출하도록 한다.    */
// 어노테이션을 할 경우 간단하다.
@WebServlet(value = "/annotation-lifecycle", loadOnStartup = 1)
public class LifeCycleTestServlet extends HttpServlet {

    /*  각 메소드 호출 푓수를 카운트 할 목적의 필드   */
    ...
    public void init() throws ServletException {
        /*  서블릿 컨테이너에 의해 호출되며, 최초 요청 시에만 실행하고 두 번째 요청부터는 호출하지 않는다.   */
